...
- each table file should be published in separate "table" sub-folder under one of supported "table category" folders
  ...
  - path to supported sub-folder should be communicated to the team that supports publishing service
  ...
  - access to each sub-folder is restricted via IAM roles defined as part of this code:
    - one particular publisher service (via IAM role) as configured in input variable table_publishers to Put objects (to minimize the risk of publishing several tables under the same sub-folder, causing unexpected table naming in Redshift)
    - crawler that processes "table category" folder
    - Redshift spectrum external schema (via IAM role chaining; on the level of "table category" folder)
...
- defines key access policy with 3 main levels of access:
  ...
  - encrypt and decrypt access (to facilitate Reads of the encrypted contents)
    - same account and services access (account's root and service principles without conditions)
...
All the variables are lists of strings that may have 2 types of principle definitions:
...
### glue.tf
For this implementation only very basic Glue infrastructure would is defined, that does not utilize some valuable Glue feature s(e.g. LakeFormation for fine-grained access control):
- Glue Data Catalog databases that correspond to "table category" folders 
- Crawlers that each only processes one "table category" folder
...
### redshift.tf
It is expected that Redshift cluster's endpoint is available for network connections to port 5439 from the location where terraform code is been applied (e.g. using VPN connection).
...
Data sources mentioned above using separate provider configuration pointing to the Redshift's account to simplify Demo deployment. 
...
- Redshift Cluster's master user is used to deploy external schemas into cluster. This is not recommended for the real scenarios, separate user with appropriate privileges should be created and configured as string for brainly/redshift provider. Also, the role configured should allow to generate temporary credentials for this particular user. 
...
- each external schema is assigned 2 roles:
...
### redshift_iam.tf
This code generates IAM roles in Glue's account that provide required access to Redshift's external schemas per each "table category" defined
These IAM roles are defined so only Redshift Cluster's "allowassume" role could assume them, hence no other principal will not be allowed to benefit from these permissions.
...
- Associated required more generic permissions (e.g. required for List operations)
